...
def get_train_data(data_path, 
    ...
    tdv = (duration - average_dur.reshape(-1,1))/average_dur.reshape(-1,1)
...
"""
Functions below are based on the example 
...
def sequential_transforms(*transforms):
    def func(txt_input):
        txt_input = list(txt_input) # tokenization
        for transform in transforms:
            txt_input = transform(txt_input)
        return txt_input
    return func

# add BOS/EOS and create tensor for input 
def tensor_transform(token_ids):
    return torch.cat((torch.tensor([BOS_IDX]),
                      torch.tensor(token_ids),
                      torch.tensor([EOS_IDX])))

def yield_iter(input, target):
  res = []
  for i in range(len(input)):
    res.append([input[i], target[i]])
  return res
...
def translate(model, src_sentence, text_transform, char_transform):
    model.eval()
    src = text_transform['input'](src_sentence).view(-1, 1)
    num_tokens = src.shape[0]
    src_mask = (torch.zeros(num_tokens, num_tokens)).type(torch.bool)
    tgt_tokens = greedy_decode(
        model,  src, src_mask, max_len=35, start_symbol=BOS_IDX).flatten()
    return "".join(char_transform['target'].lookup_tokens(list(tgt_tokens.cpu().numpy()))).replace("<bos>", "").replace("<eos>", "").replace("<pad>","")

def pred_score(input_test, target_test, transformer, text_transform, char_transform):
  score = 0
  flag = -1
  for i in range(len(input_test)):
    
    pred = translate(transformer, input_test[i], text_transform, char_transform)
    score += int(pred == target_test[i])
    if flag == -1 and pred != target_test[i]:
        print(f'true:{target_test[i]}, pred: {pred}')
        flag = 1
  print(f'score for the testing set is: {score/(i+1)}')
  return score/(i+1)
