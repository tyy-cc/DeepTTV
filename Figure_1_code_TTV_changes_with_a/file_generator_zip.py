...
mass_nontransit_upper = 10 * Jupiter_in_Solar  # M_Earth -> solar mass
...
while count_test < num_test:
    ...
    if USE_2013:
        ...
        mass_nontransit = 198.8 * Earth_in_Solar  
    else:
    ...
    #################### a ################################
    if USE_2013:
        a_nont_val = ((22.3395/DaysOfYear)**2 * mass_star)**(1/3)
        a_nont_diff = np.random.uniform(a_nont_val*0.0001, a_nont_val*0.1)
        a_nont_sign = np.random.choice(np.array([-1, 1]))
        a_nontransit = a_nont_val + a_nont_diff * a_nont_sign 
    else:
    ...
    if USE_2013:
        ...
        omega_nontransit = (g_nontransit_deg - O_nontransit*180/pi) * pi/180 
    else:
    ...
    if USE_2013:
        ...
        mean_ano_nontransit = (l_nontransit_deg - g_nontransit_deg) * pi/180
    else:
